...
const verifyOTP = async (req, res) => {
    try {
        ...
        if (UserOTPVerificationRecords.length === 0) {
            return res.json({ error: "Account record doesn't exist or has been verified already. Please sign up or log in." });
        }
...
const registerUser = async (req, res) => {
    ...
    try {
        const { FirstName, LastName, Age, Birthday, Address, Status, Gender, ContactNumber, email, password } = req.body;
...
const addStudent = async (req, res) => {
    ...
    try {
        const { LRN, FirstName, MiddleName, LastName, Age, Level, Section, Birthday, Address, MotherTongue, Nationality, Gender, } = req.body;
