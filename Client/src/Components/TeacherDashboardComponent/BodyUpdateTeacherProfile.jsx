...
const BodyUpdateTeacherProfile = () => {
  ...
  const editTeacher = async (e) => {
    e.preventDefault();
    const { FirstName, MiddleName, LastName, Department, Age, Section, Birthday, Address, Nationality, Gender, ContactNumber, email } = data
      try {
        const {data} = await axios.patch(`updateTeacher/${id}`, {
          FirstName, MiddleName, LastName, Department, Age, Section, Birthday, Address, Nationality, Gender, ContactNumber, email
        })
        if(data.error){
